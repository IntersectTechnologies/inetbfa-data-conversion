...
from zipline.finance.commission import PerShare, PerTrade, PerDollar, PerRand
...
DEFAULT_CAPITAL_BASE = float("1.0e7")
...
class TradingAlgorithm(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        :Optional:
            ...
            capital_base : float <default: 1.0e7>
        ...
        self.slippage = VolumeShareMinSlippage()
        self.commission = PerRand()
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def _create_data_generator(self, source_filter, sim_params=None):
        ...
        with_benchmarks = date_sorted_sources(benchmark_return_source, with_tnfms)
    ...
    def run(self, source, overwrite_sim_params=True,
        ...
        # check source type and wrap in corresponding zipline source
        if isinstance(source, list):
            if overwrite_sim_params:
                warnings.warn("""List of sources passed, will not attempt to extract sids, and start and end
 dates. Make sure to set the correct fields in sim_params passed to __init__().""", UserWarning)
        ...
        with ZiplineAPI(self):
            ...
            # perf dictionary
            perfs = []
            for perf in self.gen:       # AlgorithmSimulator.transform
                perfs.append(perf)
