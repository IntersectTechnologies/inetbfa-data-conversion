...
DEFAULTS = {
    ...
    'source': 'yahoo',
    'symbols': 'AAPL',
    'root': os.path.join('H:\\','Werk','repos','data', 'processed', 'jse','market','daily')
}
...
def parse_args(argv, ipython_mode=False):
    ...
    parser.add_argument('--source', choices=('yahoo','file'))
...
def run_pipeline(print_algo=True, **kwargs):
    ...
    if kwargs['source'] == 'yahoo':
        ...
        source = zipline.data.load_from_file(
            stocks=symbols, start=start, end=end, root=root)
