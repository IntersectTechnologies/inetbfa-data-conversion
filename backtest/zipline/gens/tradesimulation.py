...
class AlgorithmSimulator(object):
    ...
    def transform(self, stream_in):
        ...
        stream_in - from data generator
        """
        # Initialize the mkt_close
        mkt_open = self.algo.perf_tracker.market_open
        mkt_close = self.algo.perf_tracker.market_close

        # inject the current algo snapshot time to any log record generated.
        with self.processor.threadbound(): # Logbook 
            for date, snapshot in stream_in:

                self.simulation_dt = date
                self.algo.on_dt_changed(date)

                # If we're still in the warmup period.  Use the event to update our universe, but don't yield any perf messages,
                ...
                if date < self.algo_start:
                    ...
                    # Perf messages are only emitted if the snapshot contained a benchmark event.
                    ...
                    if self.algo.perf_tracker.emission_rate == 'minute':
                        if date == mkt_close:
                            ...
                            if mkt_close <= self.algo.perf_tracker.last_close:
                                try:
                                    mkt_open, mkt_close = trading.environment.next_open_and_close(mkt_close)

                                except trading.NoFurtherDataError:
                                    # If at the end of backtest history,
                                    # skip advancing market close.
                                    pass
                                self.algo.perf_tracker.handle_intraday_market_close(
                                             mkt_open,
                                             mkt_close)
